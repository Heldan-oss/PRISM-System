...
* CSS custom properties, system stylesheets, and packaged runtime image assets.
* The `Afacad Flux` web font loaded through a CSS `@import`, with `sans-serif` as the fallback.
* Foundry VTT notifications and chat messages.
* A JSON Actor data template.
* No JavaScript runtime dependencies, package manager, compiler, bundler, or build step.

The CSS font import is a runtime network dependency. The interface must remain usable when the remote font cannot be loaded and the configured fallback is used.
...
│   │
│   ├── ui/
│   │   ├── character_sheet_bg.jpg
│   │   └── chat_messsage_bg.jpg
│   │
│   ├── system.json
│   └── template.json
│
...
Runtime visual assets are stored in:

```text
prism/ui/
```

The current required background assets are:

```text
prism/ui/character_sheet_bg.jpg
prism/ui/chat_messsage_bg.jpg
```

These files are part of the Foundry package and must be committed, distributed, and tested with the rest of the runtime source. The current chat-background filename contains the spelling `messsage`; CSS references and packaged filenames must match exactly until both are deliberately renamed together.
...
Runtime UI assets under `prism/ui/` are source files and must be committed. They are distinct from documentation images under `docs/images/`.
...
```text
/systems/prism/ui/character_sheet_bg.jpg
/systems/prism/ui/chat_messsage_bg.jpg
```

Image assets are not registered separately in `system.json`; they must exist at the referenced package paths.

`variables.css` currently loads:

```css
@import url("https://fonts.googleapis.com/css2?family=Afacad+Flux:wght@400;700&display=swap");
```

The shared font variable is:

```css
--prism-font: "Afacad Flux", sans-serif;
```

Contributors must preserve a usable fallback and review the legal, privacy, offline, and packaging implications before changing the font source.
...
The stylesheet currently enforces these minimum window dimensions:
...
selector. This means the background and base heading or paragraph typography apply to all chat messages while the PRISM system is active, not only messages generated by `PrismActorSheet`. PRISM-specific result layout remains scoped under `.prism-chat-card` and `.prism-chat-label`.

The chat background is loaded from:
...
Packaged visual assets are stored in:
...
The current stylesheet imports `Afacad Flux` from Google Fonts:

```css
@import url("https://fonts.googleapis.com/css2?family=Afacad+Flux:wght@400;700&display=swap");
```

The system-wide font variable is:

```css
--prism-font: "Afacad Flux", sans-serif;
```

The fallback must remain usable when the external font request is blocked, unavailable, or offline. A change to the font source must document licensing, redistribution, privacy, network, and fallback implications.

### Current Background Assets

The Anomaly sheet window uses:
...
Background images currently use `background-size: cover`. The sheet background is centered and repeated, while the chat background is positioned at the bottom and repeated.

### Current Visual Direction

The current visual direction uses:

* `Afacad Flux` for sheet and chat typography, with a generic sans-serif fallback.
* A packaged textured background for the Anomaly sheet.
* A separate packaged textured background for chat messages.
* Square controls and chat containers through zero border radii.
...
The Actor-sheet window has a CSS minimum size of:

```text
760 × 860
```

while `actor-sheet.mjs` opens it at `800 × 900` by default.

### CSS Scope and Interaction Rules
...
* Preserve readable contrast over both packaged background images.
* Preserve working fallback colors when a background image fails to load.
* Use valid Foundry system paths for packaged assets.
* Avoid embedding unauthorized or unlicensed visual material.

Do not move presentation rules into Handlebars or JavaScript when CSS can express them safely.
...
* The `160 × 160` image area renders correctly.
* The packaged sheet background loads from `/systems/prism/ui/character_sheet_bg.jpg`.
* Text, inputs, and controls remain readable over the sheet background.
* The `Afacad Flux` font loads when network access is available.
* The `sans-serif` fallback remains usable when the font request is unavailable.
...
* Missing templates.
* Missing stylesheets.
* Missing runtime UI assets.
* Failed external font requests.
...
### UI Background Does Not Load

Confirm that the installed package contains:
...
Then confirm the CSS URLs match the filenames exactly:
...
Check the browser Network panel for `404` responses. The current chat filename contains `messsage`; using `chat_message_bg.jpg` in only the CSS or only the filesystem will break the background.

Also confirm that `ui/` was included in `prism.zip`.

### Web Font Does Not Load

Check the browser Network panel for the Google Fonts stylesheet and font requests.

The current import is defined at the beginning of:

```text
prism/styles/variables.css
```

A failed request must not make the interface unusable because `--prism-font` includes `sans-serif` as its fallback.

Network restrictions, content blockers, privacy settings, offline use, or changes to the remote service may prevent the external font from loading.
...
The current JavaScript project runs directly from source and does not require a package manager, compiler, bundler, or build step.

The visual layer currently has one external runtime network dependency:

```text
Google Fonts — Afacad Flux
```

It is loaded by `prism/styles/variables.css` through a remote CSS `@import`. The interface must remain usable with the declared `sans-serif` fallback.

Before adding or changing a dependency, package manager, compiler, bundler, framework, remote font, or externally hosted asset, explain:

* The problem it solves.
* Why existing code or packaged assets are insufficient.
...
Packaged runtime assets under:

```text
prism/ui/
```

are source files and must be committed. They are not generated release output.
...
* Review third-party dependencies and remotely loaded resources.
* Document external network requests introduced by CSS or JavaScript.
* Verify that packaged fonts and images may be redistributed.
...
* Confirm that sheet and chat background assets load.
* Test the interface with the remote font available and unavailable.
* Check the browser console and Network panel.
* Review the package for unauthorized content.
* Verify licensing and redistribution rights for fonts and images.
...
* `prism/ui/character_sheet_bg.jpg` exists.
* `prism/ui/chat_messsage_bg.jpg` exists.
* CSS asset URLs match the packaged filenames exactly.
...
* The sheet background loads.
* Chat messages use the expected background and remain readable.
* English and Italian work.
* The interface remains usable with the fallback font.
* No blocking console errors or missing-asset requests appear.
...
The current interface imports `Afacad Flux` from Google Fonts at runtime.

This introduces:

* A network dependency.
* Reduced control over offline availability.
* Potential privacy or content-blocking concerns.
* A difference between online rendering and fallback-font rendering.
...
The current chat background asset is named:

```text
chat_messsage_bg.jpg
```

The filename and CSS reference are functional but contain a spelling inconsistency. A rename must update the packaged file, every CSS reference, documentation, and release tests in the same focused change.
...
* The console and Network panel are checked.
* Missing assets and failed font requests are tested when visual resources change.
...
* Third-party font and image licensing is documented.
* External runtime requests and their fallback behavior are documented.
